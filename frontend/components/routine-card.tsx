    ...
    <Card className={cn("w-full mb-4 transition-all", isDone && "opacity-60 bg-muted/30")}> 
      ...
      {card.state === "due" && (
        <CardFooter className="grid grid-cols-2 gap-2 pt-0">
          <Button variant="outline" onClick={() => onSkip(card.taskInstanceId)} disabled={loading}>
            <X className="h-4 w-4 mr-2" />
            건너뛰기
          </Button>
          <Button onClick={() => onComplete(card.taskInstanceId)} disabled={loading}>
