...
export default function CalendarPage() {
    ...
    const handleAction = async (action: 'complete' | 'skip', id: string) => {
        // Actions on past/future dates? Spec allows it for completion (history) potentially, but usually for "due" states.
        ...
        try {
            ...
            toast.success(action === 'complete' ? "완료되었습니다!" : "건너뛰었습니다.");
            // Refresh
            const dateStr = format(date!, "yyyy-MM-dd");
            const res = await fetchClient(`/api/today?date=${dateStr}`);
            setCards((res as TodayResponse).cards);
                    ...
                    {loading ? (
                        ...
                        cards.map(card => (
                            <RoutineCard
                                ...
                                loading={loading} // block while refreshing
                            />
                        ))
                    ) : (
